...
| `Ctrl+→` / `Ctrl+←` | 60秒 シーク (Seek 60 seconds) |
| **再生速度** | |
| `C` | 再生速度を0.1倍上げる |
| `X` | 再生速度を0.1倍下げる |
| **動画の選別** | |
...
### ビルド方法・実行可能ファイルの場合
- `scripts\install-windows.ps1 -Build` を実行
- ビルド後、`dist\wagom-player` の内容が `C:\Program Files\wagom-player` にインストールされます
...
```powershell
.\scripts\install-windows.ps1 -Build
```

`install-windows.ps1` はビルドを通常ユーザーで実行し、`C:\Program Files\wagom-player` へのコピー時だけ管理者権限で再実行します。コピー前に `dist\wagom-player\wagom-player.exe` と `_internal` を検証し、一時ディレクトリへステージングしてから既存インストールを置き換えます。失敗した場合はバックアップから戻します。

ビルドだけを行う場合は、従来通り次のコマンドも使えます。

```bat
scripts\build_windows.bat
```

`build_windows.bat` はビルド前に起動中の `wagom-player.exe` を終了し、古い `dist` / `build` を削除してから PyInstaller で作り直します。
