...
from wagom_player.infrastructure.logger import log_message
from wagom_player.infrastructure.single_instance import (
    SingleInstanceServer,
    acquire_primary_instance_lock,
    create_single_instance_server,
    send_to_existing_instance,
)
from wagom_player.ui.main_window import MainWindow
from wagom_player.ui.theme import (
    apply_app_icon,
    apply_dark_theme,
...
def _claim_via_send_listen(initial_file: Optional[str], lock):
    ...
    """
    if send_to_existing_instance(initial_file):
        log_message("起動済みのwagom-playerへ要求を転送して終了します。")
    ...
    if single_instance_server is None:
        if send_to_existing_instance(initial_file):
            log_message("起動済みのwagom-playerへ要求を転送して終了します。")
            return None, True, lock
        log_message("単一インスタンス用サーバーを利用できないため、IPCなしで続行します。")
    return single_instance_server, False, lock
...
def _claim_single_instance(initial_file: Optional[str]):
    ...
    if not lock.is_primary:
        ...
        if _forward_or_take_over(initial_file, lock):
            ...
            return None, True, lock
        if lock.is_primary:
            log_message("主インスタンスが終了したため、処理を引き継ぎます。")
        else:
            # 所有権を取れない(primary が生存したまま応答しない等)。このまま続行すると
            # primary が生きたまま別ウィンドウを開くため、二次プロセスは終了する。
            log_message("主インスタンスが応答しないため、二次インスタンスを終了します。")
            return None, True, lock
...
def _create_player_window(initial_file: Optional[str]) -> MainWindow:
    diagnostics.heartbeat()
    diagnostics.start_hang_monitor()
    return MainWindow(file=initial_file)
