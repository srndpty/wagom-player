...
### PageUp / PageDown 巡回の手動確認
Windows 論理順に近い並び替えで PageUp / PageDown が巡回することを確認するには、以下の手順を試してみてください。

1. 例として `File1.mp4`, `file-3.mp4`, `file2.mp4`, `File10.mp4` のように数値と大文字小文字が混在する動画を同じフォルダに置きます。
2. そのフォルダ内のいずれかの動画を開くと、内部で Windows 論理順キー（`StrCmpLogicalW` が利用可能ならそれを使用、利用できない環境ではそれに近いフォールバック）が適用されます。
3. `PageDown` を押していくと `file-3.mp4 → File1.mp4 → file2.mp4 → File10.mp4` の順で進み、`PageUp` で逆順に戻ることを確認してください。
4. シャッフルを無効にした状態で確認すると、ディレクトリスキャン結果の並びをそのまま辿れるため挙動を把握しやすくなります。
